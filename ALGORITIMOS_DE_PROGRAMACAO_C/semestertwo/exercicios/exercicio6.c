...
int main() {
  ...
  for (int i = 0; i < 10 * 10 * 4; i += 4) {
      pchar2[i + 3] = 0;  // Zera o canal vermelho
  }
  ...
  // soluçao:
  for (int i = 0; i < 10 * 10 * 4; i += 4) {
      // Obtém os valores dos canais R, G e B
      unsigned char r = pchar3[i + 3];
      unsigned char g = pchar3[i + 2];
      unsigned char b = pchar3[i + 1];

      // Calcula a média
      unsigned char gray = (r + g + b) / 3;

      // Atribui o valor da média a R, G e B
      pchar3[i + 3] = gray; // Canal vermelho
      pchar3[i + 2] = gray; // Canal verde
      pchar3[i + 1] = gray; // Canal azul
  }
  // insira o seu código acima
  ...
  // soluçao:
    for (int y = 0; y < 10; y++) {
        for (int x = 0; x < 10; x++) {
            // Calcula a posição do pixel no array
            int index = (y * 10 + x) * 4;

            // Verifica se o pixel está na diagonal principal ou secundária
            if (x == y || x == (9 - y)) {
                // Define o pixel como preto (R=0, G=0, B=0)
                pchar4[index + 3] = 0;  // Canal vermelho
                pchar4[index + 2] = 0;  // Canal verde
                pchar4[index + 1] = 0;  // Canal azul
            } else {
                // Define o pixel como branco (R=255, G=255, B=255)
                pchar4[index + 3] = 255;  // Canal vermelho
                pchar4[index + 2] = 255;  // Canal verde
                pchar4[index + 1] = 255;  // Canal azul
            }
        }
    }
